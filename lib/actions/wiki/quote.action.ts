...
export async function createQuote(
  ...
  try {
    const insertQuery = `
        INSERT INTO quotes (content, from_character_id, to_character_id, created_at)
        VALUES ($1, $2, $3, NOW())
        RETURNING id
      `;
    const values = [
...
export async function listQuotes(): Promise<IQuoteSmall[]> {
  const client = await pool.connect();
  try {
    const query = `
        SELECT
    ...
    const quotes: IQuoteSmall[] = res.rows.map((row) => ({
      id: row.id,
      content: row.content,
      fromCharacter: {
        id: row.from_character_id,
