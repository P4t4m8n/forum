...
export async function createReligion(
  ...
  try {
    ...
    const insertReligionQuery = `
        ...
        RETURNING *
      `;
    const religionValues = [religionDto?.name];

    const res = await pool.query(insertReligionQuery, religionValues);
    const religion = res.rows[0];
...
export async function getReligions(): Promise<IReligionSmall[]> {
  const client = await pool.connect();
  try {
    const query = `
        SELECT*
